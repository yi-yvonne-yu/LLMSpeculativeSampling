...
class KVCacheModel():
    def __init__(self, model : torch.nn.Module, temperature : float = 1, top_k : int = 0, top_p : float = 0,
                 use_medusa_head : bool = False, medusa_head_index : int = 0) -> None:
        self._model = model
        ...
        self._top_k = top_k
        self._top_p = top_p
        self._use_medusa_head = use_medusa_head
        self._medusa_head_index = medusa_head_index
    ...
    def _run_model(self, input_ids : torch.Tensor):
        if self._use_medusa_head:
            medusa_logits, outputs, _ = self._model(
                input_ids,
            ...
            logits = medusa_logits[self._medusa_head_index]
            past_key_values = self._extract_past(outputs)
        else:
            outputs = self._model(
                input_ids,
                past_key_values=self._past_key_values,
    ...
    def _forward_with_kvcache(self, input_ids : torch.Tensor, use_debug = True) -> torch.Tensor:
        if self._past_key_values is None:
            # Prefill or re-prefill when cache is absent.
            logits, past_key_values = self._run_model(input_ids)
            ...
            not_cached_q, past_key_values = self._run_model(last_input_id)
    ...
    def rollback(self, end_pos : int):
        ...
        # Cache objects (e.g., DynamicCache) handle cropping themselves.
        if hasattr(self._past_key_values, "crop"):
            self._past_key_values = self._past_key_values.crop(end_pos)
        else:
